...
$user_name = \DB::table('users')->where('id',$form_data->user_id)->first();
if($user_name){
...
$user_id = DB::table('forms')->where('form_id',$id)->first()->assigned_to;
	...
	$retail_email = DB::table('users')->where('id',$user_id)->first()->email;
	  ...
	  if($retail_email){
		 ...
		 Mail::to($retail_email)->send(new Send_To_CBC());  	
	  }
...
$arr = array (
  ...
  'BranchName' => explode('|',$bank_details->branch_name)[1],
  ...
  'PlaceOfBirth' => explode('|',$customer_details->pob_country)[1],
  'POBCity' => explode('|',$customer_details->pob_city)[1],
  'Occupation' => $customer_details->occupation,
  'CountryOfResidence' => explode('|',$customer_details->country1)[1],
  'CORCity' => explode('|',$customer_details->city1)[1],
  'ResidentNationalOf' => $customer_details->qq,
  'DAOFBankName' => explode('|',$bank_details->bank_name)[1],
  'DAOFBankNameInvestment' => explode('|',$investment_details->bank_name)[1],
...
public function push_to_crm_json(Request $request){
