        ...
        if ($postdata) {
            if (!array_key_exists('title', $postdata)) {
                ...
                if ($getDevices) {
                    ...
                    curl_setopt($ch, CURLOPT_HTTPHEADER, array('Authorization:key=AIzaSyBujSaWiUpzE3wXflE0ttqgs5i8WWJxhOQ', 'Content-Type:application/json'));
                    $result = curl_exec($ch);
                    return $this->output
                        ->set_content_type('Content-Type: application/json')
                        ->set_status_header(200)
            ...
            if ($getDevices) {
                ...
                foreach ($getDevices as $device) {
                    ...
                    if ( strtolower($device['device_type']) == 'ios')
                        $postString['ios']['registration_ids'][] = $device['device_id'];
                    else if ( strtolower($device['device_type']) == 'android')
                        $postString['android']['registration_ids'][] = $device['device_id'];
                }
                ...
                $postString['android']['priority'] = 'high';
                $postString['android']['content-available'] = '1';
                $postdata['android']['sound'] = 'dafault';

                $postString['ios']['priority'] = 'high';
                $postString['ios']['content-available'] = '1';
                $postdata['ios']['sound'] = 'dafault';
                print_r ($postString['android']);
                // print_r ($postString['ios']);
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, "https://fcm.googleapis.com/fcm/send");
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                curl_setopt($ch, CURLOPT_POST, 1);
                ...
                curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($postString['ios']));
                curl_setopt($ch, CURLOPT_HTTPHEADER, array('Authorization:key=AIzaSyBujSaWiUpzE3wXflE0ttqgs5i8WWJxhOQ', 'Content-Type:application/json'));
                $result = curl_exec($ch);
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, "https://fcm.googleapis.com/fcm/send");
				...
				curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
                curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
                curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($postString['android']));
                curl_setopt($ch, CURLOPT_HTTPHEADER, array('Authorization:key=AIzaSyBujSaWiUpzE3wXflE0ttqgs5i8WWJxhOQ', 'Content-Type:application/json'));
                $result = curl_exec($ch);
